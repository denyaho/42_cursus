...
/*   Updated: 2025/11/15 21:25:35 by daogawa          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	char		*buf;
	ssize_t		read_byte;
	static char	*stock[OPEN_FILE_MAX + 1];
	...
	if (buf == NULL)
		return (NULL);
	while (!stock[fd] || !ft_strchr(stock[fd], '\n'))
	{
		read_byte = read(fd, buf, BUFFER_SIZE);
		if (read_byte == -1)
			return (free_handle(&stock[fd], buf), NULL);
		else if (read_byte == 0)
			break ;
		buf[read_byte] = '\0';
		stock[fd] = ft_strcat_safe(stock[fd], buf);
		if (!stock[fd])
			return (free_handle(&stock[fd], buf), NULL);
	}
	if (read_byte == 0 && (stock[fd] == NULL || stock[fd][0] == '\0'))
		return (free_handle(&stock[fd], buf), NULL);
