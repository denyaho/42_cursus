...
def process_gamedata_streaming(raw_game_logs: list) -> None:
    """
    A generator that yields formatted game event strings
    from raw logs.
    """
    for event_cnt in range(len(raw_game_logs)):
        log = raw_game_logs[event_cnt]
        if log["event_type"] == "login":
            yield (
                f"Event {event_cnt + 1}: Player {log['player']}"
                f" (level {log['data']['level']}) login"
            )
        elif log["event_type"] == "level_up":
            yield (
                f"Event {event_cnt + 1}: Player {log['player']}"
                f" (level {log['data']['level']}) leveled up"
            )
        elif log["event_type"] == "death":
            yield (
                f"Event {event_cnt + 1}: Player {log['player']}"
                f" (level {log['data']['level']}) died"
            )
        elif log["event_type"] == "kill":
            yield (
                f"Event {event_cnt + 1}: Player {log['player']}"
                f" (level {log['data']['level']}) killed monster"
            )
        elif log["event_type"] == "item_found":
            yield (
                f"Event {event_cnt + 1}: Player {log['player']}"
                f" (level {log['data']['level']}) found treasure"
            )
        elif log["event_type"] == "logout":
            yield (
                f"Event {event_cnt + 1}: Player {log['player']}"
                f" (level {log['data']['level']}) logged out"
            )


def analytics_gamedata(raw_game_logs: list) -> tuple:
    """
    Perform basic analytics on the game logs
    ...
    for log in raw_game_logs:
        if log["event_type"] == "level_up":
            levelup_events += 1
        if log["event_type"] == "item_found":
            treasure_events += 1
        if log["data"]["level"] >= 10:
            high_level_players += 1
    return (treasure_events, levelup_events, high_level_players)


def process_gamedata_constant(raw_game_logs: list) -> None:
    """
    Processes and prints game events directly
    without using a geenrator
    """
    event_cnt = 0
    for log in raw_game_logs:
        if log["event_type"] == "login":
...
def display_gamedata_analysis(raw_game_logs: list) -> None:
    """
    Execustes the analysis workflow and compares
    streaming vs constant processing
    """
    print("=== Game Data Stream Processor ===")
    print()
    print(f"Processing {len(raw_game_logs)} game events...")
    print()
    for data in process_gamedata_streaming(raw_game_logs):
        print(data)
    print()
    print("=== Stream Analytics ===")
    treasure, levelup, highlevel = analytics_gamedata(raw_game_logs)
    print(f"Total events processed: {len(raw_game_logs)}")
    print(f"High-level players (10+): {highlevel}")
    ...
    time_streaming = end - start
    start = time.perf_counter()
    process_gamedata_constant(raw_game_logs)
    end = time.perf_counter()
    time_constant = end - start
    print()

    print("Memory usage: Constant")
    print(f"Procesing time: {time_constant:.6f} seconds")
    print("Memory usage: Streaming")
    print(f"Processing time: {time_streaming:.6f} seconds")
    print()
...
if __name__ == "__main__":
    ...
    display_gamedata_analysis(raw_game_logs)
    ...
    def get_primes(number: int) -> int:
        """
        A generator that yields the first 'n' prime numbers
        """
        num = 2
        count = 1
        while count <= number:
    ...
    def fibonacci_generator(number: int) -> int:
        """
        A generator that yields the first 'n' numbers of the
        Fibonacci sequence
        """
        d1, d2 = 0, 1
        d3 = d1 + d2
